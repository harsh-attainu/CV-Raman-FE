...
form.addEventListener("submit", function (event) {
  ...
  const name_span = document.createElement("span"); //<span></span>
  const delete_span = document.createElement("span"); //<span></span>

  //add text content
  name_span.textContent = new_book_name; //<span>new book name</span>
  delete_span.textContent = "delete"; //<span>delete</span>

  //add class name
  // name_span.classList.add('name');
  name_span.className = "name"; //<span class="name">new book name</span>
  delete_span.className = "delete"; //<span class="delete">delete</span>
  ...
  new_book_element.value = ""; //clear the input
  console.log("book added successfully");
  addNewBookReq(new_book_name);
});
...
search_input.addEventListener("keyup", function (event) {
  ...
  const books = document.querySelectorAll("#book-list ul li"); //dom elements
...
const hideCheckbox = document.querySelector("#show-checkbox");

hideCheckbox.addEventListener("change", function (event) {
  console.log(event);
  if (event.target.checked) {
    list.classList.add("hidden"); //display:none;
  } else {
    list.classList.remove("hidden");
  }
});

// run the fetchallbooks function when html has been loaded
document.addEventListener("DOMContentLoaded", fetchAllBooks());

// get the books from the server and add it to the dom
async function fetchAllBooks() {
  let result = await fetchAllBooksReq();
  console.log(result);
  let book_list = document.querySelector("#book-list ul"); // ul

  if (result.code == 200 && result.books.length>0) {
    console.log("everyhting is ok. adding books now");

    let all_the_books_html = "";

    result.books.forEach(function (book) {
      console.log(book.name, " was added");

      all_the_books_html += `<li>
            <span class="name">${book.name}</span>
            <span class="delete">delete</span>
        </li>`;
    });

    console.log("updating the dom now");
    book_list.innerHTML = all_the_books_html;
  } else {
    book_list.innerHTML = `<li>
        No books created till now. Use the field below to add one now!
    </li>`;
  }
}

// a simple 'fetch' network request to get all the books
async function fetchAllBooksReq() {
  try {
    const options = {
      method: "GET",
    };
    let req = await fetch("http://localhost:3022/books/all", options);
    return req.json();
  } catch (e) {
    console.log(e);
    ...
    book_list.innerHTML = `<li>
        Error fetching books. Please refresh to try again
    </li>`;
  }
}

async function addNewBookReq(name) {
  const data = {
    name: name,
  };
  const options = {
  ...
  let req = await fetch("http://localhost:3022/books/create", options);
